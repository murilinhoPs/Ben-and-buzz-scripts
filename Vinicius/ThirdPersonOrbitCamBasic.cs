...
public class ThirdPersonOrbitCamBasic : MonoBehaviour
{
    ...
    void Update()
    {
        if (CutscenesCameras.Instance.canMoveCamera)
        {
            // Get mouse movement to orbit the camera.
            // Mouse:
            horizontalAimingSpeed = OptionsManager.sensiblity;
            verticalAimingSpeed = OptionsManager.sensiblity;
            ...
            // Deixa aqui, estou testando
            //Vector3 currentPos = defaultPos;
            //RaycastHit hit;
            //Vector3 dirTmp = parentTransform.TransformPoint(defaultPos) - referenceTransform.position;
            //if (Physics.SphereCast(referenceTransform.position, collisionOffset, dirTmp, out hit, 0.3f))
            //{
            //    currentPos = (directionNormalized * (hit.distance - collisionOffset));
            //}

            //var newCamPos = currentPos + player.position + (camYRotation * smoothPivotOffset) + (aimRotation * smoothCamOffset);

            //cam.position = Vector3.Lerp(cam.position, newCamPos, Time.deltaTime * smooth);
        }
    }
    ...
    bool ViewingPosCheck(Vector3 checkPos, float deltaPlayerHeight)
    {
        ...
        if (Physics.SphereCast(checkPos, 0.12f, target - checkPos, out RaycastHit hit, relCameraPosMag))
        {
            // ... if it is not the player...
            if (hit.transform.gameObject.tag != "Robot" && hit.transform.gameObject.tag != "Human" && hit.transform.gameObject.tag != "Cabeca" && !hit.transform.GetComponent<Collider>().isTrigger)
            {
    ...
    bool ReverseViewingPosCheck(Vector3 checkPos, float deltaPlayerHeight, float maxDistance)
    {
        ...
        if (Physics.SphereCast(origin, 0.12f, checkPos - origin, out RaycastHit hit, maxDistance))
        {
